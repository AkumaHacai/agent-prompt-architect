...
Текущий каталог скиллов целевого агента является единственным источником истины.

Приведённые ниже таблицы содержат известные паттерны маршрутизации и примеры специализированных навыков, а не гарантированно установленный набор.

Никогда не включай имя скилла в готовый промпт только потому, что оно присутствует в этом справочнике:
- Проверь, что скилл подтверждён пользователем или доступен в целевой агентной среде.
- Если нужного скилла нет в среде, не выдумывай его вызов, а сформулируй задачу в терминах стандартных инструментов и практик либо предложи установить недостающий скилл через `find-skills`.
- Выбирай минимальный достаточный набор, не перегружая контекст.
